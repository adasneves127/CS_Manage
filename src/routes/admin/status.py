    ...
    with db_connection() as conn:
        ...
        if conn.is_invoice_admin(user.seq):
            ...
            return send_template(
                "admin/finance/status.liquid", statuses=statuses,
                page="Finance"
            )
        else:
    ...
    with db_connection() as conn:
        ...
        if conn.is_invoice_admin(user.seq):
            if "id" in request.json:
                conn.update_finance_status(
                    request.json["id"], request.json["name"], session["user"]
                )
                return "OK", 201
            else:
                conn.create_finance_status(request.json["name"],
                                           session["user"])
            return "OK", 201
        else:
            raise exceptions.InvalidPermissionException()
...
def get_docket_statuses():
    with db_connection() as conn:
        ...
        if conn.is_invoice_admin(user.seq):
            ...
            return send_template(
                "admin/finance/status.liquid", statuses=statuses, page="Docket"
            )
        else:
            raise exceptions.InvalidPermissionException()
...
def post_docket_status():
    with db_connection() as conn:
        ...
        if conn.is_invoice_admin(user.seq):
            if "id" in request.json:
                conn.update_docket_status(
                    request.json["id"], request.json["name"], session["user"]
                )
                return "OK", 201
            else:
                conn.create_docket_status(request.json["name"],
                                          session["user"])
            return "OK", 201
        else:
            raise exceptions.InvalidPermissionException()
